...
def geraPlot(arquivo, comMedia, labelcores):
    ...
    fig1, ax1 = plt.subplots(
        figsize=(max((maxnomes) // 6, 12), max(5 + len(nomes) // 4, 8))
    )
    ...
    if len(nomes) == len(dados_verificados):
        ...
        (
            ...
            media,
            nomes,
            numero_linha_media_referencia,
        ) = colocareferencianofim(dados_verificados, familias, media, nomes)
        ...
        if cols > 20 and len(titulo) < 40:
            ax1.set_title(
                titulo, fontsize=tamanho_texto_super_grande, fontweight="bold"
            )
            ...
            ax1.set_title(
                titulo, fontsize=tamanho_texto_super_pequeno, fontweight="bold"
            )
            ax1.set_xlabel(
                eixoX, fontsize=tamanho_texto_super_pequeno, fontweight="bold"
            )
            ax1.set_ylabel(
                eixoY, fontsize=tamanho_texto_super_pequeno, fontweight="bold"
            )
        elif len(titulo) > 40:
