...
def run():
    ...
    eletronica = [
        "Placa queimada/defeito",
        "botão/ botoeira com defeito",
        "Falha sensor",
        "PLC travado/queimado",
        "Preventiva",
        "Calibração",
        "outros",
    ]
    ...
    if len(tipo) == 0:
        outro = Tipo_equipamento(
            nome_tipo="Outros", sigla="OUT", descricao_tipo="Outros equipamentos"
        )
        outro.save()
    ...
    try:
        caminho = os.path.join(
            ...
            "banco Migrado",
            "NovoBanco",
            "planilha_mestra_ativos_2.0v.exp.xlsm",
        )
        ...
        tipos_variavel = {
            ...
            "Fabricante": str,
            "Moeda": str,
            "Custo de aquisição": float,
            "Agência financiadora": str,
            "ID_grupo": str,
            "Responsável": str,
            "Potência elétrica": str,
            "Unidade potencia elétrica": str,
            "Tensão elétrica minima (V)": int,
            "Tensão elétrica máxima (V)": int,
            "Detalhe da alimentação elétrica (bivolt, trifásico, etc)": str,
            "Outras alimentações (ar, água, etc)": str,
            "Modelo": str,
            "Spec1": str,
            "Unidade Spec1": str,
            "Spec2": str,
            ...
            "Nacionalidade": str,
            "Ano_compra": str,
            "Projeto_financiador": str,
            "Sist_patrimonio": str,
            "Patrimonio": str,
            "Finalidade": str,
            "inter_calib": str,
            "Condição Equipamento": str,
            "Data Atualização": str,
            "Pasta_arquivos": str,
            "OBS": str,
            "endereço": str,
        }
    ...
    for counter, registro in enumerate(dados):
        ...
        buscalocal = Local_instalacao.objects.filter(
            laboratorio=laboratorio, predio=predio, piso=piso, sala=sala
        )
        if len(buscalocal) == 0:
            local = Local_instalacao(
        ...
        if len(buscatipo) == 0:
            ...
            tipo = buscatipo[0]
        buscaFabricante = Fabricante.objects.filter(
            nome_fabricante=registro["Fabricante"].capitalize()
        )
        ...
        if tensao:
            tensao += "V"
        potencia = str(registro["Potência elétrica"]) + str(
            registro["Unidade potencia elétrica"]
        )
        ...
        equipamento = Equipamento(
            nome_equipamento=nome.capitalize(),
            ...
            codigo=codigo,
            custo_aquisição=valor,
            custo_aquisição_currency=moeda,
            responsavel=responsavel.capitalize(),
            potencia_eletrica=potencia,
            nacionalidade=nacionalidade,
            data_ultima_atualizacao=hoje,
            tensao_eletrica=tensao,
            projeto_compra=projeto,
        ...
        if registro["Pasta_arquivos"]:
            nomePasta = os.path.join(
                BASE_DIR,
                "banco Migrado",
                "NovoBanco",
                "arquivos",
                registro["Pasta_arquivos"],
            )
        else:
            nomePasta = None
        print(nomePasta)
        if os.path.isdir(nomePasta):
            ...
            for arquivo in listaArquivos:
                ...
                if os.path.isfile(detalheArquivo):
                    ...
                    media1 = Media(
                        nome=arquivo, documentos=destino, equipamento=equipamento
                    )
                    media1.save()
